...
class Agent:
    ...
    def build_actor(self):
        ...
        model.compile(optimizer=Adam(lr=10e-4), loss=[proximal_policy_optimization_loss(actual_value=actual_value, old_prediction=old_prediction, predicted_value=predicted_value)])
        model.summary()
    ...
    def transform_reward(self):
        if self.episode % 1 == 0:
            print('Episode #', self.episode, '\tfinished with reward', np.array(self.reward).sum(), '\tAverage Noisy Weights', np.mean(self.actor.get_layer('output').get_weights()[1]))
